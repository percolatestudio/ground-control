...
Meteor.startup(function() {
  if (Posts.find().count() === 0) {
    ...
    Posts.insert({
      ...
      body: [
        "We are hard at work polishing up Meteor Accounts, our full-featured auth system that supports login with Facebook and Google, or with secure passwords. Many Meteor developers are already using it on our pre-release auth branch and contributing code: we've merged pull requests for Twitter and Weibo login services, with more on the way. See the Getting Started with Auth page on our wiki and the email traffic on the meteor-core list if you want to get an early look.",
        "One result of that effort is Meteor 0.4.1's new email smart package for sending email messages, which Meteor Accounts uses to send password recovery and welcome emails. When running locally, emails prints to the console for ease of debugging. In production, Email.send() will work with any standard SMTP server.",
        ...
        "Meteor 0.4.1 includes some additional changes. We've upgraded Node.js from 0.6 to 0.8, the latest stable release line of the JavaScript engine that Meteor's server-side components run on. We've made our API more consistent with JavaScript standards by converting the last few old APIs from inch_worm spellings to camelCase (though the old names continue to work for now). And we incorporated a slew of other improvements and bug fixes from the Meteor community; see History.md for full details."
      ].join('\n\n')      
    });
